...
existed_files1 <- list.files(paired_vcf_dir, pattern = "\\.vcf.gz$")
existed_files2 <- list.files(alternative_paired_vcf_dir, pattern = "\\.vcf.gz$")
existed_files <- c(existed_files1, existed_files2)
...
for(id in 1:num_files){
  ...
  if(grepl("a_", all_groupIDs[paired_IDs[id]])){
