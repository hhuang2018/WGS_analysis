...
HLA_typings <- read.csv(file = "../../2018WGS/R/R_Output/available_cases_HLA.csv")
...
avaialble_IDs_HLA_typing <- HLA_typings

#avaialble_IDs_HLA_typing <- HLA_typings[(HLA_typings$nmdp_rid %in% available_IDs$R_D_ID),]
num_groups <- dim(HLA_typings)[1]
group_type <- sapply(1:num_groups, function(x) Outcome$agvhi24[Outcome$bmt_case %in% avaialble_IDs_HLA_typing$bmt_case_num[x]])
group_type[group_type == 1] <- "aGVHD"
group_type[group_type == '0'] <- "non-GVHD"
